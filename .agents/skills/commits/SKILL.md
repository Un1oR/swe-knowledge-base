...
- Перед любым коммитом, amend, rebase, squash или другим переписыванием истории
  получи явное разрешение пользователя.
...
## Подпись и отказ от подписи
- Сохраняй требование подписи коммитов: не отключай `commit.gpgsign`, не добавляй
  `--no-gpg-sign` и не меняй ключ подписи без явного указания пользователя.
- Если подпись не прошла из-за отказа пользователя, недоступного аппаратного ключа,
  pinentry, GPG/SSH-agent или другой ошибки, остановись и сообщи пользователю, что
  операция не завершена из-за проблемы с подписью.
- Не обходи отказ от подписи неподписанным коммитом, amend, rebase continuation,
  cherry-pick continuation или любым другим способом. Сначала нужна успешная подпись
  либо новое явное решение пользователя.
- После amend/rebase/squash быстро проверь результат через `git log --show-signature`
  или эквивалентную проверку статуса подписей.
